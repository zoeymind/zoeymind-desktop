...
在设计过程中主动构建并打磨项目的 domain model。这是 _active_ discipline：挑战术语、发明 edge-case scenarios，并在概念成形的当下写入 glossary 和 decisions。（仅仅读取 `CONTEXT.md` 来获取词汇，不是这个 skill；那只是任何 skill 都能做的一行习惯。这个 skill 用于改变 model，而不是消费 model。）

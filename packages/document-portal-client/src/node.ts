...
function descriptorPath(): string {
  if (process.platform === "darwin") return join(homedir(), "Library", "Application Support", "com.zoeymind.desktop", "document-portal-broker.json")
  if (process.platform === "win32") return join(process.env.LOCALAPPDATA ?? join(homedir(), "AppData", "Local"), "com.zoeymind.desktop", "document-portal-broker.json")
  return join(process.env.XDG_DATA_HOME ?? join(homedir(), ".local", "share"), "com.zoeymind.desktop", "document-portal-broker.json")
}
...
export async function loadDocumentPortalDescriptor(): Promise<DocumentPortalDescriptor> {
  try {
    ...
    if (!isValidDocumentPortalDescriptor(descriptor)) throw new Error("invalid")
    return descriptor
  } catch { throw new Error(DOCUMENT_PORTAL_UNAVAILABLE) }
}

export async function requestDocumentPortal(tool: DocumentPortalTool, input: unknown, descriptor: DocumentPortalDescriptorLoader = loadDocumentPortalDescriptor): Promise<unknown> {
  const endpoint = await descriptor()
  let response: Response
  try { response = await fetch(`http://127.0.0.1:${endpoint.port}/v1/document-portal`, { method: "POST", headers: { authorization: `Bearer ${endpoint.token}`, "content-type": "application/json" }, body: JSON.stringify({ tool, input }) }) } catch { throw new Error("ZoeyMind Document Portal is unavailable. The desktop app may have closed; reopen it and retry.") }
  return response.json()
}

export const __test__ = { isValidDocumentPortalDescriptor }

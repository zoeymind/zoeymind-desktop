...
class MiniMap {
  ...
  declare startViewPos: { x: number; y: number }
  declare currentState
  declare isViewBoxMousedown: boolean
  ...
  calculationMiniMap(boxWidth, boxHeight) {
    let { svg, rect, origWidth, origHeight, scaleX, scaleY } = this.mindMap.getSvgData({
    ...
    viewBoxStyle.right =
      Math.max(0, ((_rectX2 - origWidth) / _rectWidth) * actWidth) + miniMapBoxLeft

    viewBoxStyle.top = Math.max(0, (-_rectY / _rectHeight) * actHeight) + miniMapBoxTop
    viewBoxStyle.bottom =
      Math.max(0, ((_rectY2 - origHeight) / _rectHeight) * actHeight) + miniMapBoxTop

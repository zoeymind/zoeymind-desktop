...
class Drag extends Base {
  ...
  async onMouseup(e) {
    ...
    // 停止自动移动
    if (
      autoMoveWhenMouseInEdgeOnDrag &&
      (this.mindMap as unknown as { select: unknown }).select
    ) {
  ...
  createCloneNode() {
    if (!this.clone) {
      ...
      if (this.beingDragNodeList.length > 1) {
        ...
        (this.mindMap.otherDraw as unknown as { add(el: unknown): void }).add(
          this.clone,
        );
      ...
      this.placeHolderLine = (this.mindMap.otherDraw.path() as unknown as SVGEl)
        .stroke({
          color:
            (dragPlaceholderLineConfig as { color: string; width: number })
              .color || lineColor,
          width: (dragPlaceholderLineConfig as { color: string; width: number })
            .width,
        })
        .fill({ color: "none" });
      // 添加横向分割线指示器，用于显示节点内部的上下分割
      this.horizontalIndicator = (
        this.mindMap.otherDraw.line() as unknown as SVGEl
      )
        .stroke({
          color:
            (dragPlaceholderLineConfig as { color: string; width: number })
  ...
  handleVerticalCheck(node, checkList, isReverse = false) {
    ...
    // 兄弟节点检测范围：节点本体+左侧扩展
    const siblingZone =
      nodeRect.left - leftExtraArea <= mouseMoveX &&
      nodeRect.right >= mouseMoveX;
    // 子节点检测范围：节点右侧区域
    const childZone =
      mouseMoveX > nodeRect.right &&
      mouseMoveX <= nodeRect.right + childZoneWidth;
    ...
    if (siblingZone) {
      ...
      if (
        ...
        let checkIsPrevNode =
          ...
          mouseMoveY <=
            nodeRect.bottom + (nextBrotherOffset > 0 ? nextBrotherOffset : 0);

        // 上半部分：从节点顶部及其扩展区域到节点中心
        let checkIsNextNode =
          mouseMoveY >=
            nodeRect.top - (prevBrotherOffset > 0 ? prevBrotherOffset : 0) &&
          mouseMoveY < nodeCenter;
      ...
      if (
        ...
        let checkIsPrevNode =
          nextBrotherOffset > 0 // 距离下一个兄弟节点的距离大于0
            ...
            : mouseMoveX <= nodeRect.right &&
              mouseMoveX >= nodeRect.right - halfWidth; // 否则在当前节点内右侧1/2区间判断
        let checkIsNextNode =
          prevBrotherOffset > 0 // 距离上一个兄弟节点的距离大于0
            ...
            : mouseMoveX <= nodeRect.left + halfWidth &&
              mouseMoveX >= nodeRect.left - leftExtraArea; // 包含节点左侧额外检测区域
        ...
        if (checkIsPrevNode) {
          if (
            ([RIGHT_FISHBONE, RIGHT_FISHBONE2] as string[]).includes(layout)
          ) {
            this.nextNode = node;
          } else {
            this.prevNode = node;
  ...
  setPlaceholderRect({
    ...
    node = node.fakeClone();
    const tmpNode = (
      this.beingDragNodeList[0] as unknown as {
        fakeClone(): Record<string, unknown>;
      }
    ).fakeClone();
    ...
    this.placeHolderExtraLines.forEach((line) => {
      ...
      line
        .stroke({
          color: (dragPlaceholderLineConfig as { color: string; width: number })
            .color,
          width: (dragPlaceholderLineConfig as { color: string; width: number })
            .width,
        })

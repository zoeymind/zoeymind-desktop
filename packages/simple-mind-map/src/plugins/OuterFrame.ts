...
class OuterFrame {
  ...
  declare static defaultStyle: Record<string, unknown>
  declare mindMap
  declare draw
  declare isNotRenderOuterFrames: boolean
  declare textNodeList
  declare outerFrameElList
  declare activeOuterFrame
  declare textEditNode: HTMLElement | null
  declare showTextEdit: boolean
  // Dynamically injected methods from outerFrameTextMethods
  declare createText: (el: unknown, cur: unknown, range: unknown) => unknown
  declare getText: (node: unknown) => string
  declare styleTextShape: (shape: unknown, style: unknown) => void
  declare styleText: (textNode: unknown, style: unknown) => void
  declare onScale: () => void
  declare showEditTextBox: (g: unknown) => void
  declare setIsShowTextEdit: (val: boolean) => void
  declare removeTextEditEl: () => void
  declare hideEditTextBox: () => void
  declare updateTextEditBoxPos: (g: unknown) => void
  declare renderText: (
    str: string,
    rect: unknown,
    textNode: unknown,
  ...
  constructor(opt: Record<string, unknown> = {}) {
    this.mindMap = opt.mindMap as unknown
    this.draw = null
      ...
      cur => {
        ...
        if (outerFrameList && outerFrameList.length > 0) {
          outerFrameList.forEach(({ nodeList, range }) => {
            ...
            const { left, top, width, height } = getNodeListBoundingRect(nodeList)
            if (
  ...
  setActiveOuterFrame(el, node, range, textNode) {
    ...
    if (!this.getText(this.getNodeRangeFirstNode(node, range))) {
      this.renderText(this.mindMap.opt.defaultOuterFrameText, el, textNode, node, range)
    }
  ...
  // 创建外框元素
  createOuterFrameEl(x, y, width, height, styleConfig: Record<string, unknown> = {}) {
  ...
  // 设置外框样式
  styleOuterFrame(el, styleConfig: Record<string, unknown>) {

...
class KeyboardNavigation {
  ...
  onKeyup(dir) {
    if (this.mindMap.renderer.activeNodeList.length > 0) {
      ...
      if (this.mindMap.opt.keyboardNavigationMoveToCenter) {
        this.mindMap.execCommand('GO_TARGET_NODE', root)
      } else {
        this.activateNodeWithoutMoving(root)
      }
    }
  }
  ...
  focus(dir) {
    ...
    if (targetNode) {
      if (this.mindMap.opt.keyboardNavigationMoveToCenter) {
        ...
        this.activateNodeWithoutMoving(targetNode)
      }
    }
  }

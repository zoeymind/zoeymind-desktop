...
export interface DefaultOpt {
  ...
  customNoteContentShow: { show: () => void; hide: () => void } | null
  ...
  handleNodePasteImg:
    | ((blob: Blob) => Promise<{ url: string; size: { width: number; height: number } }>)
    | null
  ...
  customHandleLine:
    | ((
        node: unknown,
        line: unknown,
        options: { width: number; color: string; dasharray: string }
      ) => void)
    | null
  ...
  addCustomContentToNode: {
    create: (node: unknown) => { el: HTMLElement; width: number; height: number }
  ...
  addContentToHeader: (() => { el: HTMLElement; cssText?: string; height: number } | null) | null
  addContentToFooter: (() => { el: HTMLElement; cssText?: string; height: number } | null) | null
...
export const defaultOpt = {
  ...
  // 节点数量上限，0表示不限制，超过该数量将阻止添加新节点并触发 node_limit_exceeded 事件
  maxNodeCount: 0,
  // 最大历史记录数
  maxHistoryCount: 500,
  ...
  */
  ...
  // 同层级节点等宽对齐，开启后同一深度层级的节点宽度会统一为该层最大宽度
  ...
  */
  ...
  // 键盘导航时是否移动画布到节点中心
  // 当为true时，使用键盘箭头键导航会将选中的节点移动到画布中心；当为false时，只改变节点选中状态，画布保持不变
  ...
  */
  ...
  // 导出png、svg、pdf时会获取画布上的svg数据进行克隆，然后通过该克隆的元素进行导出，如果你想对该克隆元素做一些处理，比如新增、替换、修改其中的一个元素，那么可以通过该参数传递一个处理函数，接收svg元素对象，处理后，需要返回原svg元素对象。
  ...
  */
  ...
  rainbowLinesConfig: {
    open: false, // 是否开启彩虹线条
    colorsList: [] as string[] // 自定义彩虹线条的颜色列表，如果不设置，会使用默认颜色列表
    /*
    [
      'rgb(255, 213, 73)',
      'rgb(255, 136, 126)',
  ...
  // 【Painter】插件
  // 是否只格式刷节点手动设置的样式，不考虑节点通过的主题的应用的样式

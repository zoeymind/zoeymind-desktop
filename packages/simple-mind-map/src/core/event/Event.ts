...
class Event extends EventEmitter {
  ...
  declare opt: { mindMap: import('../../index').default; [key: string]: unknown }
  declare mindMap: import('../../index').default
  declare isLeftMousedown: boolean
  declare isRightMousedown: boolean
  declare isMiddleMousedown: boolean
  declare mousedownPos: { x: number; y: number }
  declare mousemovePos: { x: number; y: number }
  declare mousemoveOffset: { x: number; y: number }

  //  构造函数
  constructor(opt: Event['opt'] = { mindMap: undefined! }) {
    super()
    this.opt = opt
  ...
  onMousemove(e) {
    ...
    if (
      ...
      (useLeftKeySelectionRightKeyDrag ? this.isRightMousedown : this.isLeftMousedown)
    ) {
      e.preventDefault()

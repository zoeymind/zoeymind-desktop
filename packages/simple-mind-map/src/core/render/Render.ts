...
class Render {
  ...
  declare mindMap: MainMindMap;
  declare themeConfig: Record<string, unknown>;
  declare renderTree: Record<string, unknown> | null;
  declare reRender: boolean;
  declare isRendering: boolean;
  declare hasWaitRendering: boolean;
  declare nodeCache: Record<string, import("./node/MindMapNode").default>;
  declare lastNodeCache: Record<string, import("./node/MindMapNode").default>;
  declare renderSourceList: unknown[];
  declare renderCallbackList: ((...args: unknown[]) => void)[];
  declare activeNodeList: import("./node/MindMapNode").default[];
  declare emitNodeActiveEventTimer: ReturnType<typeof setTimeout> | null;
  declare renderTimer: ReturnType<typeof setTimeout> | null;
  declare root: import("./node/MindMapNode").default | null;
  declare textEdit: TextEdit;
  declare beingCopyData: unknown;
  declare highlightBoxNode: Polygon | null;
  declare highlightBoxNodeStyle: Record<string, unknown> | null;
  declare lastActiveNodeList: unknown[];
  declare layout;

  //  构造函数
  constructor(opt: Record<string, unknown> = {}) {
    this.mindMap = opt.mindMap as MainMindMap;
    // 渲染树，操作过程中修改的都是这里的数据
  ...
  //   绑定事件
  bindEvent() {
    const { openPerformance, openRealtimeRenderOnNodeTextEdit } =
      this.mindMap.opt;
    const performanceConfig = this.mindMap.opt.performanceConfig as {
      time: number;
    };
    // 画布点击事件清除当前激活节点列表
  ...
  registerCommands() {
    ...
    this.mindMap.command.add(
      ...
      this.moveNodeDataToIndex,
    );
    this.patchNodeDataTree = this.patchNodeDataTree.bind(this);
    this.mindMap.command.add("PATCH_NODE_DATA_TREE", this.patchNodeDataTree);
    // 删除节点
  ...
  handlePaste(event) {
    ...
    let img = null;
    let text = "";
    (Array.from(items) as DataTransferItem[]).forEach((item) => {
      ...
      try {
        ...
        if (text) {
          ...
          if (this.mindMap.opt.customHandleClipboardText) {
            try {
              const res = await (
                this.mindMap.opt.customHandleClipboardText as (
                  text: string,
                ) => unknown
              )(text);
              if (!isUndef(res)) {
          ...
          if (smmData) {
            ...
            if (textArr.length > 1 && handleIsSplitByWrapOnPasteCreateNewNode) {
              (handleIsSplitByWrapOnPasteCreateNewNode as () => Promise<void>)()
                .then(() => {
                  this.mindMap.execCommand(
                    "INSERT_MULTI_CHILD_NODE",
                    [],
  ...
  removeNode(appointNodes = []) {
    ...
    if (root) {
      ...
      for (let i = 0; i < list.length; i++) {
        const node = list[i] as import("./node/MindMapNode").default;
        const currentEditNode = this.textEdit.getCurrentEditNode();
        if (
          currentEditNode &&
          (currentEditNode as import("./node/MindMapNode").default).getData(
            "uid",
          ) === node.getData("uid")
        ) {
          // 如果当前节点正在编辑中，那么先完成编辑
  ...
  // 原子修改完整数据树，不依赖节点当前是否已渲染。
  patchNodeDataTree(apply) {
    if (typeof apply !== "function") {
      throw new Error("PATCH_NODE_DATA_TREE requires a mutation function");
    }
    apply(this.renderTree);
    this.mindMap.render();
  }
  ...
  insertFormula(formula, appointNodes = []) {
    ...
    if (
      ...
      !(this.mindMap as unknown as Record<string, unknown>)["formula"]
    )
      return;
    appointNodes = formatDataToArray(appointNodes);
    const list = appointNodes.length > 0 ? appointNodes : this.activeNodeList;
    list.forEach((node) => {
      (
        (this.mindMap as unknown as Record<string, unknown>)["formula"] as {
          insertFormulaToNode: (node: unknown, formula: unknown) => void;
        }
  ...
  setNodeCustomPosition(node, left = undefined, top = undefined) {
    let nodeList = node ? [node] : this.activeNodeList;
    nodeList.forEach((item) => {
  ...
  // 定位到指定节点
  // callback 保证必然被调用: 成功传节点实例, 失败传 null.
  // (之前失败路径静默 return, 调用方包 Promise 时永远不 resolve, 只能等外层超时 — 已修)
  goTargetNode(node, callback: (...args: unknown[]) => void = () => {}) {
  ...
  // 从完整数据树中根据uid查找节点数据(包括未渲染/折叠的节点)
  findNodeDataByUid(uid) {

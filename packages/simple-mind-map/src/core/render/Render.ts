...
class Render {
  ...
  //   绑定事件
  bindEvent() {
    const { openPerformance, openRealtimeRenderOnNodeTextEdit } = this.mindMap.opt
    const performanceConfig = this.mindMap.opt.performanceConfig as { time: number }
    // 画布点击事件清除当前激活节点列表
  ...
  emitNodeActiveEvent(node = null, activeNodeList = [...this.activeNodeList]) {
    const isChange = !checkNodeListIsEqual(this.lastActiveNodeList, activeNodeList)
    if (!isChange) return
  ...
  insertNode(openEdit = true, appointNodes = [], appointData = null, appointChildren = []) {
    ...
    const { defaultInsertSecondLevelNodeText, defaultInsertBelowSecondLevelNodeText } =
      this.mindMap.opt
      ...
      try {
        ...
        if (text) {
          ...
          if (this.mindMap.opt.customHandleClipboardText) {
            try {
              const res = await (
                this.mindMap.opt.customHandleClipboardText as (text: string) => unknown
              )(text)
              if (!isUndef(res)) {
                useDefault = false
  ...
  removeNode(appointNodes = []) {
    ...
    if (root) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        if (
          currentEditNode &&
          (currentEditNode as import('./node/MindMapNode').default).getData('uid') ===
            node.getData('uid')
        ) {
          // 如果当前节点正在编辑中，那么先完成编辑
          this.textEdit.hideEditTextBox()
  ...
  addGeneralization(data, openEdit = true) {
    ...
    const nodeList = this.activeNodeList.filter(node => {
      return !node.isRoot && !node.isGeneralization && !node.checkHasGeneralization()
    })

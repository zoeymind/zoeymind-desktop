...
class Style {
  ...
  static setBackgroundStyle(el, themeConfig) {
    ...
    if (!Style.cacheStyle) {
      ...
      backgroundStyleProps.forEach(prop => {
        if (Style.cacheStyle) {
          Style.cacheStyle[prop] = style[prop as keyof CSSStyleDeclaration] as string
        }
      })
    }
    // 设置新样式
  ...
  static removeBackgroundStyle(el) {
    ...
    backgroundStyleProps.forEach(prop => {
      ;(el.style as Record<string, string>)[prop] = (Style.cacheStyle as Record<string, string>)[
        prop
      ]
    })
  ...
  shape(node) {
    ...
    if (styles.gradientStyle) {
      if (!this._gradient) {
        this._gradient = (
          this.ctx.nodeDraw as unknown as { gradient(type: string): Record<string, unknown> }
        ).gradient('linear')
      }
      ;(
        this._gradient as unknown as { update(cb: (add: Record<string, unknown>) => void): void }
      ).update((add: Record<string, unknown>) => {
        ;(add as unknown as { stop(pos: number, color: unknown): void }).stop(0, styles.startColor)
        ;(add as unknown as { stop(pos: number, color: unknown): void }).stop(1, styles.endColor)
      })
      ;(this._gradient as unknown as { from(...args: number[]): { to(...args: number[]): void } })
  ...
  //  连线
  line(line, { width, color, dasharray }: Record<string, unknown> = {}, enableMarker?, childNode?) {
  ...
  createMarker() {
    return (
      ...
      this._markerPath = (m as unknown as { path(d: string): Record<string, unknown> }).path(
        'M0,0 L2,5 L0,10 L10,5 Z'
      )
    })
  }

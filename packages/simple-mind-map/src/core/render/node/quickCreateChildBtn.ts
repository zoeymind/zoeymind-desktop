...
function showQuickCreateChildBtn() {
  ...
  if (this._quickCreateChildBtn) {
    ...
    // 图标节点
    const iconNode = SVG(icon || btnsSvg.quickCreateChild) as unknown as {
      size(w: number, h: number): unknown
      css(props: Record<string, string>): unknown
      x(n: number): unknown
      y(n: number): unknown
    }
    iconNode.size(expandBtnSize, expandBtnSize)
    iconNode.css({
      cursor: 'pointer'
    })
    iconNode.x(0)
    iconNode.y(-expandBtnSize / 2)

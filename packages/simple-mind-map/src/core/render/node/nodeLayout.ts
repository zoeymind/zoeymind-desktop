...
function getNodeRect() {
  ...
  if (this._iconData.length > 0) {
    ...
    const stackOffset = iconSize * 0.6 // 遮挡40%
    // 折叠布局的总宽度：第一个icon宽度 + 其余icon的偏移距离
    const totalIconWidth = iconSize + (this._iconData.length - 1) * stackOffset
    textContentWidth += totalIconWidth
    textContentHeight = Math.max(textContentHeight, iconSize)
    spaceCount++
  }
  // 文字
  if (this._textData) {
  ...
  // 评论标签
  if (this._commentLabelData) {
    textContentWidth += this._commentLabelData.width
    textContentHeight = Math.max(textContentHeight, this._commentLabelData.height)
    spaceCount++
  }
  // 自定义后置内容
  ...
  // 计算节点形状需要的附加内边距
  const { paddingX: shapePaddingX, paddingY: shapePaddingY } = this.shapeInstance.getShapePadding(
    _width,
    _height,
    paddingX,
    paddingY
...
function layout() {
  ...
  let imgWidth = 0
  let imgHeight = 0
  // 判断是否开启了同层级节点等宽对齐（非根节点时内容左对齐）
  const isAlignLeftForImg = this.mindMap.opt.alignSameLevelNodeWidth && !this.isRoot
  if (this._imgData) {
    ...
    switch (imgPlacement) {
      case IMG_PLACEMENT.TOP:
        if (isAlignLeftForImg) {
          this._imgData.node.x(paddingX).y(paddingY)
        } else {
      ...
      case IMG_PLACEMENT.BOTTOM:
        if (isAlignLeftForImg) {
          this._imgData.node.x(paddingX).y(height - paddingY - imgHeight)
        } else {
  ...
  if (this._iconData && this._iconData.length > 0) {
    const iconCount = this._iconData.length
    const iconSize = this.mindMap.themeConfig.iconSize
    // 折叠偏移距离：遮挡40%，即每个icon偏移60%的宽度
    const stackOffset = iconSize * 0.6

    // 反向添加，让第一个icon在最上层
    for (let i = this._iconData.length - 1; i >= 0; i--) {
      const item = this._iconData[i]
      const index = i

      // 折叠布局：遮挡40%
      const xPosition = textContentOffsetX + index * stackOffset

      item.node.x(xPosition).y((textContentHeight - item.height) / 2)

      // 保存原始索引，用于恢复层级
      item.node.attr('data-original-index', index)

      iconNested.add(item.node)
    }

    textContentNested.add(iconNested)
    // 总宽度：第一个icon宽度 + 其余icon的偏移距离
    const totalWidth = iconSize + (iconCount - 1) * stackOffset
    textContentOffsetX += totalWidth + textContentMargin
  }
  // 文字
  if (this._textData) {
  ...
  // 评论标签
  if (this._commentLabelData) {
    this._commentLabelData.node
      .x(textContentOffsetX)
      .y((textContentHeight - this._commentLabelData.height) / 2)
    textContentNested.add(this._commentLabelData.node)
    textContentOffsetX += this._commentLabelData.width + textContentMargin
  }
  // 自定义后置内容
  if (this._postfixData) {
    const foreignObject = createForeignObjectNode({
  ...
  // 判断是否开启了同层级节点等宽对齐（非根节点时内容左对齐）
  const isAlignLeft = this.mindMap.opt.alignSameLevelNodeWidth && !this.isRoot
  let translateX = 0
  let translateY = 0
  switch (imgPlacement) {
    case IMG_PLACEMENT.TOP:
      translateX = isAlignLeft ? paddingX : width / 2 - bboxWidth / 2
      translateY =
        paddingY + // 内边距
        imgHeight + // 图片高度
    ...
    case IMG_PLACEMENT.LEFT:
      ...
      translateY = isAlignLeft ? paddingY : height / 2 - bboxHeight / 2
      break
    case IMG_PLACEMENT.RIGHT:
      translateX = paddingX
      translateY = isAlignLeft ? paddingY : height / 2 - bboxHeight / 2

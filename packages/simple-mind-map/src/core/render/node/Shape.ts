...
export default class Shape {
  declare node: {
    ...
    getPaddingVale(): { paddingX: number; paddingY: number }
    style: { merge(prop: string, root?: boolean): unknown }
    shapePadding: { paddingX: number; paddingY: number }
    width: number
    height: number
  ...
  declare mindMap: {
    extendShapeList: {
      ...
      getPadding(opts: Record<string, unknown>): { paddingX: number; paddingY: number } | undefined
    }[]
  ...
  createPath(pathStr) {
    ...
    const customCreateNodePath = opt.customCreateNodePath as
  ...
  createPolygon(points) {
    const opt = this.mindMap.opt as Record<string, unknown>
    const customCreateNodePolygon = opt.customCreateNodePolygon as
  ...
  createRect() {
    ...
    let borderRadius = this.node.style.merge('borderRadius') as unknown as number
    const pathStr = `
      M${borderRadius},0
...
export const shapeList = [
  {
    name: CONSTANTS.SHAPE.RECTANGLE,
    label: '矩形'
  },
  {
    name: CONSTANTS.SHAPE.DIAMOND,
    label: '菱形'
  },
  {
    name: CONSTANTS.SHAPE.PARALLELOGRAM,
    label: '平行四边形'
  },
  {
    name: CONSTANTS.SHAPE.ROUNDED_RECTANGLE,
    label: '圆角矩形'
  },
  {
    name: CONSTANTS.SHAPE.OCTAGONAL_RECTANGLE,
    label: '八角矩形'
  },
  {
    name: CONSTANTS.SHAPE.OUTER_TRIANGULAR_RECTANGLE,
    label: '外三角矩形'
  },
  {
    name: CONSTANTS.SHAPE.INNER_TRIANGULAR_RECTANGLE,
    label: '内三角矩形'
  },
  {
    name: CONSTANTS.SHAPE.ELLIPSE,
    label: '椭圆'
  },
  {
    name: CONSTANTS.SHAPE.CIRCLE,
    label: '圆'
  }
]

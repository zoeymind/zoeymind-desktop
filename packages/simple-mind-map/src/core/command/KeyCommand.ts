...
export default class KeyCommand {
  declare opt: { mindMap: MindMap; [key: string]: unknown }
  declare mindMap: MindMap
  declare shortcutMap: Record<string, Array<(...args: unknown[]) => void>>
  declare shortcutMapCache: Record<string, Array<(...args: unknown[]) => void>>
  declare isPause: boolean
  declare isInSvg: boolean
  declare isStopCheckInSvg: boolean
  ...
  // 按键事件
  onKeydown(e) {
    const { enableShortcutOnlyWhenMouseInSvg, beforeShortcutRun, customCheckEnableShortcut } =
      this.mindMap.opt
    const checkFn =
    ...
    if (
      this.isPause ||
      (enableShortcutOnlyWhenMouseInSvg && !this.isStopCheckInSvg && !this.isInSvg)
    ) {
      return
    }

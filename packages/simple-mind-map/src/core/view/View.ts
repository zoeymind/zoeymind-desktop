...
class View {
  ...
  bind() {
    ...
    this.mindMap.event.on('mousedown', e => {
      const { isDisableDrag, mousedownEventPreventDefault } = this.mindMap.opt as {
    ...
    this.mindMap.event.on('drag', (e, event) => {
      // 按住ctrl键拖动为多选
      // 禁用拖拽
      if (e.ctrlKey || e.metaKey || (this.mindMap.opt.isDisableDrag as boolean)) {
        return
      }
      if (this.firstDrag) {
    ...
    this.mindMap.event.on('mousewheel', (e, dirs, event, isTouchPad) => {
      const {
        ...
        customHandleMousewheel?: (e: MouseEvent) => void
        mousewheelAction?: string
      ...
      if (mousewheelAction === CONSTANTS.MOUSE_WHEEL_ACTION.ZOOM || e.ctrlKey || e.metaKey) {
        ...
        if (
          isTouchPad &&
          (dirs.includes(CONSTANTS.DIR.LEFT) || dirs.includes(CONSTANTS.DIR.RIGHT))
        ) {
  ...
  //  缩小
  narrow(cx?: number, cy?: number, isTouchPad?: boolean) {
    let { scaleRatio, minZoomRatio } = this.mindMap.opt as unknown as {
      scaleRatio: number
      minZoomRatio: number
    }
  ...
  //  放大
  enlarge(cx?: number, cy?: number, isTouchPad?: boolean) {
    let { scaleRatio, maxZoomRatio } = this.mindMap.opt as unknown as {
      scaleRatio: number
      maxZoomRatio: number
    }
    scaleRatio = scaleRatio / (isTouchPad ? 5 : 1)
  ...
  // 适应画布大小
  fit(getRbox: () => undefined = () => undefined, enlarge = false, fitPadding?: number) {
    fitPadding = fitPadding === undefined ? (this.mindMap.opt.fitPadding as number) : fitPadding
    const draw = this.mindMap.draw
    const origTransform = draw.transform() as Record<string, number>
    const rect = (getRbox() || draw.rbox()) as unknown as Record<string, number>
    ...
    const newRect = (getRbox() || draw.rbox()) as unknown as Record<string, number>
  ...
  checkNeedMindMapInCanvas() {
    // 如果当前在演示模式，那么不需要限制
    if (this.mindMap.demonstrate && (this.mindMap.demonstrate.isInDemonstrate as boolean)) {
      return false
    }
    const { isLimitMindMapInCanvasWhenHasScrollbar, isLimitMindMapInCanvas } = this.mindMap.opt as {
  ...
  // 计算图形四个方向的位置边界值
  getPositionLimit() {
    const { scaleX, scaleY } = this.mindMap.draw.transform() as { scaleX: number; scaleY: number }
    const drawRect = this.mindMap.draw.rbox() as unknown as Record<string, number>
    const rootRect = this.mindMap.renderer.root.group.rbox() as unknown as Record<string, number>

...
import defaultTheme, { checkIsNodeSizeIndependenceConfig } from './theme/default'
...
class MindMap {
  ...
  // Static properties
  declare static instanceCount: number
  declare static pluginList: Array<Record<string, unknown>>
  declare static extendNodeDataNoStylePropList: (list?: string[]) => void
  declare static resetNodeDataNoStylePropList: () => void
  declare static usePlugin: (plugin: object, opt?: Record<string, unknown>) => typeof MindMap
  declare static hasPlugin: (plugin: object) => number
  declare static defineTheme: (name: string, config?: Record<string, unknown>) => void | Error
  declare static removeTheme: (name: string) => void
  //  构造函数
  /**
   *
  ...
  constructor(opt = {}) {
    ...
    */
    ...
    // 初始渲染
    this.render(this.opt.fit ? () => this.view.fit(undefined, undefined, undefined) : () => {})
  ...
  //  渲染，部分渲染
  render(callback?: (...args: unknown[]) => void, source = '') {
    this.initTheme()
    this.renderer.render(callback, source)
  }

  //  重新渲染
  reRender(callback?: (...args: unknown[]) => void, source = '') {
  ...
  //  设置主题
  setTheme(theme: string, notRender = false) {
    this.execCommand('CLEAR_ACTIVE_NODE')
    this.opt.theme = theme
    if (!notRender) {
      this.render(undefined, CONSTANTS.CHANGE_THEME)
    }
  ...
  setThemeConfig(config: Record<string, unknown>, notRender = false) {
    ...
    if (!notRender) {
      ...
      this.render(undefined, res ? '' : CONSTANTS.CHANGE_THEME)
    }
  }
  ...
  setLayout(layout: string, notRender = false) {
    ...
    if (!notRender) {
      this.render(undefined, CONSTANTS.CHANGE_LAYOUT)
    }
  ...
  execCommand(command: string, ...args: unknown[]) {
    this.command.exec(command, ...args)
  }

  // 更新画布数据，如果新的数据是在当前画布节点数据基础上增删改查后形成的，那么可以使用该方法来更新画布数据
  updateData(data: MindMapNodeTree) {
  ...
  getData(withConfig?: boolean): MindMapNodeTree | Record<string, unknown> {
  ...
  //  导出
  async export(type: string, isDownload?: boolean, name?: string, ...args: unknown[]) {
    try {
      if (!this.doExport) {
        throw new Error('请注册Export插件！')
      }
      const result = await this.doExport.export(type, isDownload, name, ...args)
      return result
    } catch (error) {
  ...
  getSvgData({
    ...
    // 保存原始信息
    const origWidth = Number(svg.width())
    const origHeight = Number(svg.height())
    ...
    if (header && headerHeight > 0) {
      ;(clone.findOne('.smm-container') as unknown as G).translate(0, headerHeight)
  ...
  removePlugin(plugin: {
    ...
    const name = String(plugin.instanceName)
    if (this[name]) {
      ;(
        this[name] as { beforePluginRemove?(): void; [key: string]: unknown }
      ).beforePluginRemove?.()
    }
    this[name] = null
  }
  ...
  destroy() {
    ...
    ;[...MindMap.pluginList].forEach(plugin => {
      const name = String(plugin.instanceName)
      if (this[name] && this[name].beforePluginDestroy) {
        this[name].beforePluginDestroy()
      }
      this[name] = null
    })
    // 解绑事件
...
// Named runtime exports with proper types
...
// 插件列表
MindMap.pluginList = []
MindMap.usePlugin = (plugin: object, opt: Record<string, unknown> = {}) => {
  const p = plugin as Record<string, unknown>
  if (MindMap.hasPlugin(plugin) !== -1) return MindMap
  p.pluginOpt = opt
  MindMap.pluginList.push(p)
  return MindMap
}

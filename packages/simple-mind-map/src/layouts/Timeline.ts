        ...
        if (isRoot) {
          ...
          if (this.layout === CONSTANTS.LAYOUT.TIMELINE2) {
            ...
            if (parent._node.dir) {
              ...
              // 节点生长方向
              newNode.dir =
                index % 2 === 0 ? CONSTANTS.LAYOUT_GROW_DIR.BOTTOM : CONSTANTS.LAYOUT_GROW_DIR.TOP
            }
          } else {
            newNode.dir = ''
          }
        ...
        if (parent && !parent.isRoot && len > 0) {
          ...
          let totalHeight =
            node.children.reduce((h, item) => {
              return (
                h + item.height + (this.getNodeActChildrenLength(item) > 0 ? item.expandBtnSize : 0)
              )
            }, 0) +
            len * marginY
  ...
  renderExpandBtnRect(rect, expandBtnSize, width, height, node) {
    if (this.layout === CONSTANTS.LAYOUT.TIMELINE) {
      ...
      if (node.dir === CONSTANTS.LAYOUT_GROW_DIR.TOP) {
        dir =
          node.layerIndex === 1 ? CONSTANTS.LAYOUT_GROW_DIR.TOP : CONSTANTS.LAYOUT_GROW_DIR.BOTTOM
      } else {

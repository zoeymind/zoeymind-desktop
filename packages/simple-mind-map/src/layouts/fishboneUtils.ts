  ...
  bottom: {
    ...
    computedLeftTopValue({ layerIndex, node, ctx }) {
      ...
      if (layerIndex > 1 && node.children) {
        ...
        let startLeft = node.left + node.width * ctx.childIndent
        let totalTop =
          node.top - (ctx.getNodeActChildrenLength(node) > 0 ? node.expandBtnSize : 0) - marginY
        node.children.forEach(item => {
          item.left = startLeft
      ...
      if (parent && parent.isRoot) {
        ...
        node.children.forEach(item => {
          ...
          let offset = hasChildren
            ? nodeTotalHeight - item.height - (hasChildren ? item.expandBtnSize : 0)
            : 0
          ...
          item.left =
            node.left +
            node.width * ctx.indent +
            (nodeTotalHeight + totalHeight2) / Math.tan(degToRad(ctx.mindMap.opt.fishboneDeg))
          totalHeight += offset

...
class Base {
  ...
  declare draw: G
  declare lineDraw: G
  declare root: MindMapNode | null
  declare lru: Lru
  declare rootNodeCenterOffset: { x: number; y: number } | null
  declare isUseLeft?: boolean
  ...
  constructor(renderer) {
    ...
    // 根节点
    this.root = null
    this.lru = new Lru(this.mindMap.opt.maxNodeCacheCount as number)
  ...
  // 计算基础值后的统一后处理（同层级节点等宽对齐等）
  // 子类的 doLayout 中，在 computedBaseValue 之后调用此方法即可
  afterComputedBaseValue() {
    this.alignSameLevelNodeWidth()
  }
  ...
  checkNodeFixChange(newNode, nodeInnerPrefixData, nodeInnerPostfixData) {
    ...
    this.mindMap.nodeInnerPrefixList.forEach(item => {
      const updateFn = item.updateNodeData as (...args: unknown[]) => unknown
      if (updateFn) {
        const isChange = updateFn(newNode, nodeInnerPrefixData)
        if (isChange) {
          isNodeInnerPrefixChange = isChange as boolean
        }
      }
    })
    // 库后置内容是否改变了
    ...
    this.mindMap.nodeInnerPostfixList.forEach(item => {
      const updateFn = item.updateNodeData as (...args: unknown[]) => unknown
      if (updateFn) {
        const isChange = updateFn(newNode, nodeInnerPostfixData)
        if (isChange) {
          isNodeInnerPostfixChange = isChange as boolean
        }
      }
    })
  ...
  createNode(data, parent, isRoot, layerIndex, index, ancestors) {
    // 创建节点
    // 库前置内容数据
    const nodeInnerPrefixData: Record<string, unknown> = {}
    this.mindMap.nodeInnerPrefixList.forEach(item => {
      const createFn = item.createNodeData as (
    ...
    // 库后置内容数据
    const nodeInnerPostfixData: Record<string, unknown> = {}
    this.mindMap.nodeInnerPostfixList.forEach(item => {
      const createFn = item.createNodeData as (
        args: Record<string, unknown>
      ) => [string, unknown] | undefined
    ...
    if (data && data._node && !this.renderer.reRender) {
      ...
      if (
        ...
        isNodeDataChange ||
        isLayerTypeChange ||
        (newNode.getData('resetRichText') && !newNode.isUseCustomNodeContent()) ||
        newNode.getData('needUpdate') ||
        isNodeInnerFixChange
      ) {
        newNode.getSize()
      ...
      newNode = new MindMapNode({
        data,
        uid: newUid,
        renderer: this.renderer as unknown as Record<string, unknown>,
  ...
  getRootCenterOffset(width, height) {
    ...
    if (initRootNodePosition[0] === CENTER && initRootNodePosition[1] === CENTER) {
      ...
      // 否则需要计算当前配置和默认配置的差值
      const tmpNode: Record<string, number> = {
        width: width,
        height: height
      }
      const tmpNode2: Record<string, number> = {
        width: width,
        height: height
      }
  ...
  createFoldLine(list) {
    ...
    const lineRadius = themeConfig.lineRadius as number
    ...
    if (len >= 3 && lineRadius > 0) {
      ...
      const isOneLine =
        ...
        (start[1].toFixed(0) === center[1].toFixed(0) && center[1].toFixed(0) === end[1].toFixed(0))
      if (!isOneLine) {
  ...
  getMarginX(layerIndex) {
    ...
    const tc = themeConfig as Record<string, { marginX: number; marginY: number }>
    const { second, node } = tc
    const hoverRectPadding = (opt.hoverRectPadding as number) * 2
  ...
  //  同层级节点等宽对齐
  //  遍历节点树，按 layerIndex 分组，将同层级节点的宽度统一为该层最大宽度
  //  同时重新计算受影响的 left 值（因为父节点宽度变化会影响子节点的 left）
  alignSameLevelNodeWidth() {
    if (!this.mindMap.opt.alignSameLevelNodeWidth) return
    if (!this.root) return
    // 1. 按层级收集所有节点
    const layerMap = new Map() // layerIndex -> [node, ...]
    ...
    // 2. 对每个层级，基于内容自然宽度找到最大值并统一
    layerMap.forEach((nodes, layer) => {
      // 跳过根节点层级（layerIndex === 0）
      if (layer === 0) return
      if (nodes.length <= 1) return
      // 使用 _contentWidth（内容自然宽度）来计算最大值
      // 这样即使某个节点之前被对齐撑大了，缩小内容后能正确收缩
      let maxWidth = 0
      nodes.forEach(node => {
        const w = node._contentWidth || node.width
        if (w > maxWidth) {
          maxWidth = w
        }
      })
      // 将同层级节点宽度统一为最大宽度，并标记需要重新渲染
      nodes.forEach(node => {
        // 缓存对齐后的宽度，用于单节点 reRender 时防止宽度收缩闪烁
        node._alignedWidth = maxWidth
        if (node.width !== maxWidth) {
          node.width = maxWidth
          // 宽度变化了，需要重新渲染节点的形状和 hover 框
          node.needLayout = true
        }
      })
    })
    // 3. 重新计算所有非根节点的 left 值（因为父节点/自身宽度可能变化了）
    const recalcLeft = node => {
      if (!node || !node.children || node.children.length === 0) return
      node.children.forEach(child => {
        if (child.hasCustomPosition && child.hasCustomPosition()) return
        if (child.isRoot) return
        const marginX = this.getMarginX(child.layerIndex)
        // 判断方向：优先使用节点的 dir 属性（MindMap 布局），其次使用布局的 isUseLeft 属性（LogicalStructure 布局）
        const isLeft = child.dir ? child.dir === 'left' : this.isUseLeft === true
        if (isLeft) {
          child.left = node.left - child.width - marginX
        } else {
          child.left = node.left + node.width + marginX
        }
        recalcLeft(child)
      })
    }
    recalcLeft(this.root)
  }
  ...
  transformPath(path) {
    ...
    const { customTransformNodeLinePath } = opt
    const transformFn = customTransformNodeLinePath as ((path: unknown) => string) | undefined
    if (transformFn) {

...
class LogicalStructure extends Base {
  ...
  adjustTopValue() {
    walk(
      ...
      (node, parent, isRoot, layerIndex) => {
        ...
        let difference =
          node.childrenAreaHeight2 - this.getMarginY(layerIndex + 1) * 2 - node.height
        if (difference > 0) {
  ...
  renderLineCurve(node, lines, style) {
    ...
    const { nodeUseLineStyle, rootLineStartPositionKeepSameInCurve, rootLineKeepSameInCurve } =
  ...
  renderGeneralization(list) {
    list.forEach(item => {
      let { left, top, bottom, right, generalizationLineMargin, generalizationNodeMargin } =

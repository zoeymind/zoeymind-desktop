...
class Fishbone extends Base {
      ...
      (node, parent, isRoot, layerIndex, index, ancestors) => {
        ...
        if (isRoot) {
          ...
          if (parent._node.dir) {
            ...
            // 节点生长方向
            newNode.dir =
              index % 2 === 0 ? CONSTANTS.LAYOUT_GROW_DIR.TOP : CONSTANTS.LAYOUT_GROW_DIR.BOTTOM
          }
          // 计算二级节点的top值
  ...
  getNodeAreaHeight(node) {
    ...
    let loop = node => {
      ...
      totalHeight +=
        node.height + (this.getNodeActChildrenLength(node) > 0 ? node.expandBtnSize : 0) + marginY
  ...
  renderLine(node, lines, style) {
    ...
    if (node.isRoot) {
      ...
      node.children.forEach(item => {
        ...
        let offset = node.height / 2 + marginY - (this.isFishbone2() ? node.height / 4 : 0)
        ...
        if (this.checkIsTop(item)) {
          ...
          line.plot(
            this.transformPath(
              `M ${nodeLineX - offsetX},${item.top - offset} L ${nodeLineX},${item.top}`
            )
          )
        }
  ...
  renderGeneralization(list) {
    list.forEach(item => {
      let { top, bottom, right, generalizationLineMargin, generalizationNodeMargin } =

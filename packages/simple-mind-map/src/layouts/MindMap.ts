...
class MindMap extends Base {
    ...
    walk(
      ...
      (cur, parent, isRoot, layerIndex, index, ancestors) => {
        ...
        if (isRoot) {
          ...
          if (parent._node.dir) {
            ...
            newNode.dir =
              newNode.getData('dir') ||
              (index % 2 === 0 ? CONSTANTS.LAYOUT_GROW_DIR.RIGHT : CONSTANTS.LAYOUT_GROW_DIR.LEFT)
          }
          // 根据生长方向定位到父节点的左侧或右侧
          newNode.left =
      ...
      (cur, parent, isRoot, layerIndex) => {
        ...
        cur._node.leftChildrenAreaHeight =
          leftChildrenAreaHeight + (leftLen + 1) * this.getMarginY(layerIndex + 1)
        cur._node.rightChildrenAreaHeight =
          rightChildrenAreaHeight + (rightLen + 1) * this.getMarginY(layerIndex + 1)

        // 如果存在概要，则和概要的高度取最大值
        let generalizationNodeHeight = cur._node.checkHasGeneralization()
          ? cur._node._generalizationNodeHeight + this.getMarginY(layerIndex + 1)
          : 0
  ...
  renderLineCurve(node, lines, style) {
    ...
    const { nodeUseLineStyle, rootLineKeepSameInCurve, rootLineStartPositionKeepSameInCurve } =
  ...
  renderGeneralization(list) {
    list.forEach(item => {
      ...
      let { top, bottom, left, right, generalizationLineMargin, generalizationNodeMargin } =

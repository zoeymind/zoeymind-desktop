...
const handleBackgroundSize = ({
  ...
  if (backgroundSize) {
    ...
    if (backgroundSizeValueArr[0]) {
      if (Array.isArray(backgroundSizeValueArr[0])) {
        ...
        if (backgroundSizeValueArr[0][1] === '%') {
          // %单位
          drawOpt.width = (Number(backgroundSizeValueArr[0][0]) / 100) * canvasWidth
          newNumberWidth = drawOpt.width
        } else {
          // 其他都认为是px单位
          drawOpt.width = backgroundSizeValueArr[0][0]
          newNumberWidth = Number(backgroundSizeValueArr[0][0])
        }
        ...
        if (backgroundSizeValueArr[1]) {
          if (backgroundSizeValueArr[1][1] === '%') {
            ...
            drawOpt.width = zoomWidth(
              imageRatio,
              (Number(backgroundSizeValueArr[1][0]) / 100) * canvasHeight
            )
          } else {
            // 其他都认为是px单位
    ...
    if (backgroundSizeValueArr[1] && Array.isArray(backgroundSizeValueArr[1])) {
      ...
      if (backgroundSizeValueArr[1][1] === '%') {
        // 高度为%单位
        drawOpt.height = (Number(backgroundSizeValueArr[1][0]) / 100) * canvasHeight
      } else {
        // 其他都认为是px单位
  ...
  if (backgroundPosition) {
    ...
    if (Array.isArray(backgroundPositionValueArr[0])) {
      ...
      if (backgroundPositionValueArr[0][1] === '%') {
        // 单位为%
        let canvasX = (Number(backgroundPositionValueArr[0][0]) / 100) * canvasWidth
        let imgX = (Number(backgroundPositionValueArr[0][0]) / 100) * imgWidth
        // 计算差值
      ...
      if (backgroundPositionValueArr[1][1] === '%') {
        // 单位为%
        let canvasY = (Number(backgroundPositionValueArr[1][0]) / 100) * canvasHeight
        let imgY = (Number(backgroundPositionValueArr[1][0]) / 100) * imgHeight
        // 计算差值

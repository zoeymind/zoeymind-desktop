...
export const handleNodeImageFromXmind = async (node, newNode, promiseList, files) => {
  if (node.image && /\.(jpg|jpeg|png|gif|webp)$/.test(node.image.src)) {
    ...
    try {
      ...
      const imageBase64 =
        ...
        (await files['resources/' + node.image.src.split('/')[1]].async('base64'))
      newNode.data.image = imageBase64
      // 如果图片尺寸不存在
      if (!node.image.width && !node.image.height) {
        const imageSize = (await getImageSize(imageBase64)) as { width: number; height: number }
...
// 导出为xmind时，处理图片数据
export const handleNodeImageToXmind = async (node, newData, promiseList, imageList) => {
  if (node.data.image) {
    // 处理异步逻辑
    let resolve = null

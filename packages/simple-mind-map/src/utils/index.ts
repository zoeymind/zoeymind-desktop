  ...
  let walk = root => {
    ...
    childNodes.forEach(node => {
      if (node.nodeType === 1) {
        ...
        // 文本节点
        root.replaceChild(
          document.createTextNode(node.nodeValue.replace(new RegExp(searchText, 'g'), replaceText)),
          node
        )
      }
    })
...
export const removeRichTextStyes = html => {
  ...
  Array.from(formulaList).forEach(el => {
    ...
    ;(el as Element).parentNode?.replaceChild(placeholder, el as Node)
  })
  ...
  if (formulaList.length > 0) {
    ...
    Array.from(els).forEach((el, index) => {
      ;(el as Element).parentNode?.replaceChild(formulaList[index], el as Node)
    })
...
export const addDataToAppointNodes = (appointNodes, data: Record<string, unknown> = {}) => {
  data = { ...data }
...
// 处理输入框的粘贴事件，会去除文本的html格式，保持撤销功能
export const handleInputPasteText = (e, text) => {
  ...
  if (document.execCommand) {
    try {
      ...
      // execCommand 失败，降级到手动处理
      console.warn('execCommand failed, falling back to manual insertion:', error)
    }
  }
  // 降级方案：手动DOM操作（原有逻辑）
...
export const transformObjectToTreeData = data => {
  ...
  // 🚀 优化1: 构建父子关系映射，避免重复查找 - O(n) 时间复杂度
  const parentMap = new Map() // uid -> parentUid
  const childrenMap = new Map() // parentUid -> [childUid, childUid, ...]
  uids.forEach(uid => {
    const node = data[uid]
    if (node.children && node.children.length > 0) {
      childrenMap.set(uid, [...node.children]) // 浅拷贝children数组
      node.children.forEach(childUid => {
        parentMap.set(childUid, uid)
      })
    }
  })
  // 🚀 优化2: 使用Map缓存节点，避免重复创建
  ...
  const createNode = uid => {
    ...
    const nodeData = data[uid]
    if (!nodeData) return null
    // 🚀 优化3: 使用浅拷贝替代深拷贝，提升性能
    const node = {
      data: { ...nodeData.data }, // 浅拷贝节点数据
      children: []
    }
...
// 全屏
export const fullScreen = (element: Record<string, (...args: unknown[]) => void>) => {
...
export const exitFullScreen = () => {
  ...
  if (doc['exitFullscreen']) {
    ;(doc['exitFullscreen'] as () => void)()
  } else if (doc['webkitExitFullscreen']) {
    ;(doc['webkitExitFullscreen'] as () => void)()
  } else if (doc['mozCancelFullScreen']) {
    ;(doc['mozCancelFullScreen'] as () => void)()
  }
}
// 创建foreignObject节点
export const createForeignObjectNode = ({
  ...
  foreignObject.node.appendChild(el as Node)
...
*/
// 递归统计节点树的节点总数
export const countNodeTree = node => {
  if (!node) return 0
  let count = 1
  if (node.children && node.children.length > 0) {

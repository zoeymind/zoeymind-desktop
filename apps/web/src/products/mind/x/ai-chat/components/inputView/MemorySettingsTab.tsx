...
import { MEMORY_MODEL_SOURCES } from "../../../ai-chat/memory/settings"
  ...
  return (
    <div className="flex flex-col gap-5 py-2">
      ...
      <div className="flex items-center justify-between gap-3">
        <Label htmlFor="memory-model-source" className="text-sm">
          {t("mindmap.aiChat.memory.modelSource")}
        </Label>
        <Select value={modelSource} onValueChange={value => value && setModelSource(value)}>
          <SelectTrigger id="memory-model-source" className="w-52">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {MEMORY_MODEL_SOURCES.map(source => (
              <SelectItem key={source.id} value={source.host}>
                {t(
                  `mindmap.aiChat.memory.modelSource${source.id === "mirror" ? "Mirror" : "Official"}`
                )}
              </SelectItem>
            ))}
          </SelectContent>

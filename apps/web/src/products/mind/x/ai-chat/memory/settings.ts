...
// v2: 之前版本接受 same-origin path 但 transformers.js 拒绝, 这里改 key 让旧值失效, 用 default
const KEY_MIRROR_HOST = 'ai-memory-mirror-host-v2'

/** 默认走后端代理 (transformers.js 要求绝对 URL, 不接受同源相对路径) */
export const DEFAULT_MIRROR_HOST =
  typeof window !== 'undefined' ? `${window.location.origin}/api/hf-proxy/` : ''

export const MIRROR_PRESETS: Array<{ label: string; value: string }> = [
  {
    label: '代理',
    value: typeof window !== 'undefined' ? `${window.location.origin}/api/hf-proxy/` : ''
  },
  { label: '镜像', value: 'https://hf-mirror.com/' }
]
/** 默认召回 3 条相关历史 */

...
import { getProject, pendingProjects, refreshProjectIndex } from "@/shared/native"

// === Desktop project index bridge ===
...
export interface DesktopProjectDB {
  getProject(id: string): Promise<DesktopProjectSummary | null>
  updateProject(id: string, patch: { name?: string; nodeCount?: number }): Promise<boolean>
}

export const projectDB: DesktopProjectDB = {
  async getProject(id) {
    if (!id) return null
    if (pendingProjects.isPending(id)) {
      const pending = pendingProjects.read(id)
      return pending
        ? { id, name: pending.title, updatedAt: pending.createdAt, createdAt: pending.createdAt }
        : null
    }
  ...
  async updateProject(id, patch) {
    ...
    await refreshProjectIndex(id, patch)
    return true
  },
}

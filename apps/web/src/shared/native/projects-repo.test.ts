...
describe("renameProjectFile", () => {
  ...
  it("renames the backing .zmind file and synchronizes its indexed path and name", async () => {
    ...
    expect(db.execute).toHaveBeenCalledWith(expect.stringContaining("SET path = $1, name = $2"), [
      "/vault/New_name.zmind",
      "New_name",
      expect.any(Number),
      "project-1",
    ])
...
describe("moveProjectToFolder", () => {
  ...
  it("changes only the virtual folder relation and leaves the file path untouched", async () => {
    ...
    expect(fs.rename).not.toHaveBeenCalled()
    expect(db.execute).toHaveBeenCalledWith(expect.stringContaining("SET folder_id = $1"), [
      "folder-2",
      expect.any(Number),
      "project-1",
    ])
